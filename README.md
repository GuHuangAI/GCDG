...
Notice that the model v12 is used in the paper, and we present a new model v13 now in which the motion query is obtained by using a linear layer to project the initial motion. You can change the model version in the above config files.
<pre><code>
For training:
sh run_cmtr.sh
