...
from datasets.test_dataloader import AISTDataset, paired_collate_fn1, paired_collate_fn2
...
def test(args, generator, testloader, device, test_dir, classifier, save_results=True, save_fig=True):
    total_loss = 0.
    epoch = args.epoch
    predict_joints = []
    target_joints = []
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm.tqdm(testloader, desc='Generating dance poses')):
            ...
            if args.network == "2tcnlstm" or args.network == 'Dancerevolution_2Transformer':
                poses = generator.generate(audio_seq, condition_motion, tgt_start, total_len=tgt_seq.shape[1])
            else:
            ...
            for idx in range(0, test_batch_size):
                ...
                if save_fig:
                    ...
                    AISTDataset.plot_smpl_poses(pred_poses, pred_trans, save_type='pred',
                                                ...
                                                test=True, test_dir=test_dir, epoch=epoch, to_video=True,
                    ...
                    # image_list = os.listdir(os.path.join(test_dir, 'mesh_frames/pred'))
                    # print(image_list[:11])
                ...
                '''
                # evaluation metrics
                predict_joints = fk.SMPLForwardKinematics().from_aa(pred_poses, pred_trans)
                target_joints = fk.SMPLForwardKinematics().from_aa(gt_poses, gt_trans)
                pos_var, vel_var = motion_div(predict_joints)
                batch_pos_frechet += pos_frechet(predict_joints, target_joints)
                batch_vel_frechet += vel_frechet(predict_joints, target_joints)
                batch_pos_var += pos_var
                batch_vel_var += vel_var
                '''
...
def run(args):
    ...
    test(args, generator, testloader, device, test_dir, classifier)
