...
import _init_paths
from evaluation.metric import calculate_metrics

from evaluation.evalution_metric import *
from datasets.loader import get_music_name, get_tempo
from datasets.dataloader import AISTDataset, paired_collate_fn1, paired_collate_fn2, paired_collate_fn3
...
parser.add_argument('--num_workers', default=4, type=int, help='number of workers for each data loader, default 4.')
...
parser.add_argument('--classifier_path', default="/export/home/chengyh/music2dance_condition/classifier/best_model_512_512dim_60frame_rnn",
...
class TTA(object):
    def __init__(self, args):
        ...
        if self.args.network == "Dancerevolution_2Transformer":
            ...
            paired_collate_fn = paired_collate_fn3

        train_dataset = AISTDataset(train_video_names, train_music_names, self.args.config.motion_dir, self.args.config.music_dir, self.args.config)
        ...
        val_dataset = AISTDataset(val_video_names, val_music_names, self.args.config.motion_dir, self.args.config.music_dir, self.args.config, sample=1000)
        self.valloader = data.DataLoader(val_dataset, self.args.config.val_batch_size, collate_fn=paired_collate_fn)

        test_dataset = td.AISTDataset(test_video_names, test_music_names, self.args.config.motion_dir, self.args.config.music_dir)
        testloader = data.DataLoader(test_dataset, self.args.config.test_batch_size, collate_fn=td.paired_collate_fn1)
        ...
        if self.args.loss_version == '1':
            self.loss_funcs = nn.L1Loss()
        else:
            self.loss_funcs = LOSS(self.args.config)

        self.optimizer = torch.optim.AdamW(filter(lambda x: x.requires_grad, model.parameters()), lr=self.args.config.leaning_rate)
        self.lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(self.optimizer,
                                                                 [int(0.6 * self.args.config.epochs), int(0.95 * self.args.config.epochs)],
                                                                 0.1)

        self.best_val_loss = float("inf")
        self.best_pos_fd = float("inf")
        self.best_beat_align = 0
        self.best_acc = 0
        self.best_fid = float('inf')
        self.best_vel_fd = float("inf")
    ...
    def run(self):
        ...
        for epoch in range(self.last_epoch, self.args.config.epochs + 1):
            ...
            if epoch % 100 == 0:
                ...
                #val_loss, metrics = self.best_val_loss, self.metrics
            ...
            if self.best_beat_align < metrics['beat_alignment'] \
                ...
                self.best_pos_fd = metrics['pos_fd']
                self.best_vel_fd = metrics['vel_fd']
        ...
        for _ in range(self.args.config.repeat_time_per_epoch):
            for batch_i, batch in enumerate(self.trainloader):
                ...
                if self.args.network != 'CrossModalTr':
                    ...
                    # out_frame = tgt_seq[:, 0]
                    out_seq = None
                genre_label_np = np.array([genres.index(x[:3]) for x in video_name])
                genre_label = torch.LongTensor(genre_label_np).to(self.device)

                self.optimizer.zero_grad()
                if self.args.saved_version in ['11', '12']:
                    genre_onehot = torch.nn.functional.one_hot(genre_label, len(genres))
                    output = self.model(audio_seq, condition_motion, tgt_seq, regressive_len=20, type='full',genre_onehot=genre_onehot)
                else:
                    output = self.model(audio_seq, condition_motion, tgt_seq, regressive_len=20, type='full')

                if self.args.loss_version == '1':
                    loss = self.loss_funcs(output[0], gold_seq)
                    loss_tuple = (loss,)
                else:
                    ...
                    if batch_i == len(self.trainloader) - 1:
                        ...
                        for idx in tqdm.tqdm(range(0, self.args.config.train_batch_size)):
                            ...
                            AISTDataset.plot_smpl_poses(pred_poses, pred_trans, save_type='pred',
                                                        fname='{}'.format(batch_i * self.args.config.train_batch_size + idx),
    ...
    def evaluate(self, epoch, args, classifier, device, save_results=True):
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(tqdm.tqdm(self.valloader, desc='Generating dance poses')):
                ...
                if self.args.network != 'CrossModalTr':
                    ...
                    self.model.eval()
                    poses = self.model(audio_seq, condition_motion, tgt_seq, type='full', eval=True)
                ...
                loss_tuple = nn.L1Loss()(poses, tgt_seq)

                total_loss.append(loss_tuple.item())
                if batch_idx == len(self.valloader) - 1:
                    # pred, gt = convert_to3d(poses, tgt_seq)
                    for idx in range(0, self.args.config.val_batch_size):
                        ...
                        """save as gif"""
                        AISTDataset.plot_smpl_poses(pred_poses, pred_trans, save_type='pred',
                                                    fname='{}'.format(batch_idx * self.args.config.val_batch_size + idx),
