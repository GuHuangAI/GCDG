...
class Model(nn.Module):
    ...
    def forward(self, audio_seq, condition_motion, tgt_seq, gt, hidden, dec_output, out_seq, epoch_i):
        ...
        # print('seq_length: ', seq_len) # 119
        vec_h, vec_c = hidden

        # print(audio_seq.shape)
        enc_outputs1 = self.encoder1(audio_seq[:, :-1])  # (b, 239, 200)
        # print('outputs1.shape: ', enc_outputs1.shape)
        enc_outputs2 = self.encoder2(condition_motion[:, :-1])  # (b, 119, 200)
        # print('outputs2.shape: ', enc_outputs2.shape)
        ...
        for i in range(seq_len):
            ...
            # print('1', dec_output.shape) # (b, 1224)
            dec_output = self.linear(dec_output)
            # print('2', dec_output.shape) # (b, 219)
            out_seq = torch.cat([out_seq, dec_output], 1)
            # print('3', out_seq.shape)
