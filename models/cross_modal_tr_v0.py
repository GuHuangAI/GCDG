...
class Encoder(nn.Module):
    ...
    def __init__(
            ...
            d_model=800, d_inner=256, dropout=0.1, seq_len=240):
        ...
        self.position_enc = nn.Embedding.from_pretrained(
            ...
            freeze=True)
        self.position = nn.Embedding(seq_len, d_word_vec)
        nn.init.normal_(self.position.weight, std=0.05)

        self.layer_stack = nn.ModuleList([
            EncoderLayer(d_model, d_inner, n_head, d_k, d_v, dropout=dropout)
            for _ in range(n_layers)])

    def forward(self, src_seq, src_pos, mask=None, return_attns=False, cross=False):
        ...
        if cross:
            ...
            enc_output = self.src_emb(src_seq) + self.position.weight.unsqueeze(0).repeat(bs, 1, 1)
...
class CMTr(nn.Module): ##Cross Modal Transformer
    def __init__(self, d_in=800, d_hidden=800, d_out=219, d_forward=1024, dropout=0.1, num_queries=120,
        ...
        self.pos_q = nn.Parameter(torch.normal(mean=0, std=0.02, size=(1, num_queries, d_hidden)))
        # self.query_embed = nn.Embedding(num_queries, d_hidden)
    ...
    def forward(self, enc_input, tgt, mask=None, return_attns=False, learnable_query=False):
        enc_slf_attn_list = []
        bs = enc_input.shape[0]
        src = self.enc_emb(enc_input)
        if learnable_query:
...
class Model(nn.Module):
    def __init__(self, encoder1, encoder2, decoder, condition_step=10,
        ...
        self.out_layer = nn.Linear(encoder1.d_model, d_out)
    ...
    def full_forward(self, audio_seq, condition_motion, tgt_seq, eval=False):  ### non-autoregressive
        ...
        out_seq = self.decoder(enc_outputs, src_pos2, mask=None, cross=True)
        out_tmp = self.out_layer(self.dropout(out_seq[0][:, :20, :]))
        # out_genre = out_tmp[:, -1, :]
    ...
    def generate(self, audio_seq, condition_motion, tgt_seq, regressive_len=1, target_len=20*60, type='full'):
