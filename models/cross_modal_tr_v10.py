...
class CMTr(nn.Module): ##Cross Modal Transformer
    def __init__(self, d_in=800, d_hidden=800, d_out=219, d_forward=1024, dropout=0.1, num_queries=60,
        ...
        self.pos_q = nn.Parameter(torch.normal(mean=0, std=0.05, size=(1, 120, d_hidden)))
        # self.query_embed = nn.Embedding(num_queries, d_hidden)
...
class Model(nn.Module):
    def __init__(self, encoder1, encoder2, decoder, condition_step=10,
        ...
        nn.init.normal_(self.query_embed.weight, std=0.1)
        ...
        #     nn.Linear(512, 10)
        #     )
        self.genre_layer = Classifier(input_size=219, emb_dim=200, hidden_size=200, num_classes=10)
        # for n, p in self.genre_layer.named_parameters():
    ...
    def full_forward(self, audio_seq, condition_motion, tgt_seq, eval=False):  ### non-autoregressive
        ...
        out_query = condition_seq
        out_seq = self.decoder(enc_outputs1, src_pos1, out_query, learnable_query=False)
        out_tmp = self.out_layer(self.dropout(out_seq))
        # out_genre = out_tmp[:, -1, :]
        genre = self.genre_layer(out_tmp)
        out_seq = out_tmp[:, :60, :] + condition_seq[:, -1, :].unsqueeze(1).repeat(1, 60, 1)
        if eval:
            return out_seq
        out_seq = self.post_process(out_seq, targets=tgt_seq)
