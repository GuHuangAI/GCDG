...
class DecoderLayer(nn.Module):
    ...
    def forward(self, dec_input, enc_input, slf_attn_mask=None, non_pad_mask=None):
        dec_input, _ = self.slf_attn(
            dec_input, dec_input, dec_input, mask=slf_attn_mask)
        dec_output, enc_slf_attn = self.slf_attn(
        ...
        return dec_output, enc_slf_attn
...
class CMTr(nn.Module): ##Cross Modal Transformer
    def __init__(self, d_in=800, d_hidden=800, d_out=219, d_forward=1024, dropout=0.1, num_queries=120,
        ...
        # self.pos_q = nn.Parameter(torch.randn(num_queries+1, d_hidden))
        # self.query_embed = nn.Embedding(num_queries, d_hidden)
    ...
    def forward(self, enc_input, enc_outputs_pos, tgt, tgt_pos, genre_emb, mask=None, return_attns=False, learnable_query=False):
        enc_slf_attn_list = []
        bs = enc_input.shape[0]
        src = self.enc_emb(enc_input) + self.position_enc(enc_outputs_pos)
        if learnable_query:
            tgt = self.tgt_emb(tgt) + genre_emb
        else:
            # tgt = self.tgt_emb(tgt) + self.pos_q.unsqueeze(0).repeat(bs, 1, 1)
            tgt = self.tgt_emb(tgt) + self.position_dec(tgt_pos) + genre_emb
        bs, _, _ = src.shape
        # if tgt == None:
...
class Model(nn.Module):
    def __init__(self, encoder1, encoder2, decoder, condition_step=10,
        ...
        # nn.init.normal_(self.query_embed.weight)
        self.genre_embedding = nn.Linear(10, decoder.d_hidden)
        self.genre_token = nn.Parameter(torch.randn(1, 1, d_out))
        self.dropout = nn.Dropout(0.1)
        self.out_layer = nn.Linear(d_hidden, d_out)
        # self.genre_layer = nn.Linear(d_hidden, 10)
    ...
    def forward(self, audio_seq, condition_motion, tgt_seq, regressive_len=20, type='auto', eval=False, genre_onehot=None):
        if type == 'full':
            out_seq = self.full_forward(audio_seq,condition_motion,tgt_seq, eval=eval, genre_onehot=genre_onehot)
        elif type == 'auto':
    ...
    def full_forward(self, audio_seq, condition_motion, tgt_seq, eval=False, genre_onehot=None):  ### non-autoregressive
        # print(src_seq[0].shape)
        ...
        genre_emb = self.genre_embedding(genre_onehot).unsqueeze(1).repeat(1, 120, 1)
        out_query = condition_seq[:, :120, :]
        out_query_pos = np.array([[pos_i + 1 for pos_i, v_i in enumerate(inst)] for inst in out_query])
        out_query_pos = torch.LongTensor(out_query_pos).to(out_query.device)
        out_seq = self.decoder(enc_outputs1, src_pos1, out_query, out_query_pos, genre_emb, learnable_query=False)
    ...
    def generate(self, audio_seq, condition_motion, tgt_seq, regressive_len=10, target_len=20*60, type='full', genre_onehot=None):
        bsz, out_len, _ = tgt_seq.size()
    ...
    def post_process(self, output, targets=None):
        ...
        output_trans = output[:, :, 216:]
        pred_smpl_out = self.smpl_forward(output_pose, output_trans)
        pred_joints = pred_smpl_out.joints[:, :24].reshape(bs, seq_len, 24, 3)
        out = [output, pred_joints]

        if targets is not None:
            bs, seq_len, _ = targets.shape
            targets = targets[:, :, :216]
            targets_trans = targets[:, :, 216:]
            target_smpl_out = self.smpl_forward(targets, targets_trans)
            target_joints = target_smpl_out.joints[:, :24].reshape(bs, seq_len, 24, 3)
