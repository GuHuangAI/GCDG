...
class CMTr(nn.Module): ##Cross Modal Transformer
    def __init__(self, d_in=800, d_hidden=800, d_out=219, d_forward=1024, dropout=0.1, num_queries=120,
        ...
        self.pos_q = nn.Parameter(torch.normal(mean=0, std=0.05, size=(1, num_queries, d_hidden)))
        # self.query_embed = nn.Embedding(num_queries, d_hidden)
    ...
    def forward(self, enc_input, enc_outputs_pos, tgt, genre_emb, mask=None, return_attns=False, learnable_query=False):
        enc_slf_attn_list = []
...
class Model(nn.Module):
    def __init__(self, encoder1, encoder2, decoder, condition_step=10,
                 d_hidden=800, num_queries=60, d_out=219,
        ...
        self.query_embed = nn.Embedding(num_queries, d_out)
        # self.
    ...
    def full_forward(self, audio_seq, condition_motion, tgt_seq, eval=False, genre_onehot=None):  ### non-autoregressive
        ...
        src_pos2 = condition_motion[1]
        enc_mask2 = get_subsequent_mask(condition_seq, self.sliding_windown_size)
        enc_outputs2, *_ = self.encoder2(condition_seq, src_pos2, mask=None)  # (b, seq_len2, dim)

        enc_outputs = torch.cat([enc_outputs1, enc_outputs2], 1)  # (b, seq_len1+seq_len2, dim)
        enc_outputs_pos = np.array([[pos_i + 1 for pos_i, v_i in enumerate(inst)] for inst in enc_outputs])
        enc_outputs_pos = torch.LongTensor(enc_outputs_pos).to(enc_outputs.device)

        genre_emb = self.genre_embedding(genre_onehot).unsqueeze(1).repeat(1, self.num_queries, 1)
        out_query = self.query_embed.weight.unsqueeze(0).repeat(bsz, 1, 1)
        out_seq = self.decoder(enc_outputs, enc_outputs_pos, out_query, genre_emb, learnable_query=True)
        out_tmp = self.out_layer(self.dropout(out_seq[:, :self.num_queries, :]))
        # out_genre = out_tmp[:, -1, :]
        # genre = self.genre_layer(out_seq[:, -1, :])
        out_seq = out_tmp + condition_seq[:, -1, :].unsqueeze(1).repeat(1, 60, 1)
        if eval:
            return out_seq
        out_seq = self.post_process(out_seq, targets=tgt_seq)
