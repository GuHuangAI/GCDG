...
class DecoderStep(nn.Module):
    ...
    def forward(self, input, hidden=None, cell=None, encoder_outputs=None):
        if (hidden is None) and (cell is None):
            ...
            # print("input shape：", input.shape) #(1, 16, 219)
            # print("hidden shape：", hidden.shape) #(1, 16, 800)
            # print("cell shape：", cell.shape) #(1, 16, 800)
            output, (hidden, cell) = self.lstm(input, (hidden, cell))
            # output, (hidden, cell) = self.lstm(input.view(input.shape[1], input.shape[0], input.shape[2]), (hidden, cell)) 
...
class LSTMDecoder(nn.Module):
    ...
    def forward(self, tgt, init_motion=None, hidden=None, cell=None, max_len=None, teacher_forcing_ratio=0.5,):
        """
        Inputs:
        ...
        # print('tgt shape: ', tgt.shape) #(seq_len, batch_size, input_dim)
        max_len = max_len if max_len is not None else tgt.shape[0]
        batch_size = tgt.shape[1]

        input = init_motion
        # print('motion: ', torch.min(init_motion), torch.max(init_motion))
        # input = tgt[0, :]
