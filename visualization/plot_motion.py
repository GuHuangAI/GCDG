...
def animate_matplotlib(
    positions, colors, titles, fig_title, parents=SMPL_PARENTS, 
    change_color_frames=None, color_after_change='g', overlay=False,
    fps=60, out_dir=None, save_type=None, fname=None, keep_frames=True,
    figsize=(10.8, 10.8), trajectory_len=10, test=False, test_dir=None, epoch=None,
    to_video=False, audio_path=None):
    """
    ...
    if epoch!=None:
        ...
        save_path = os.path.join(test_dir, 'skeleton')
    ...
    for j in range(0, seq_length):
        ...
        if test==True:
            ...
            fig.savefig(
                os.path.join(frame_path, 'frame_{:0>4}.{}'.format(j, "jpg")), dip=1000)

    line_ani.save(os.path.join(save_path, '{}'.format(save_type) + '_' + fname + ".gif"), writer='pillow', fps=fps)
    ...
    if test == True: 
        if to_video:
            ...
            ffmpeg.save_to_movie(
                ...
                os.path.join(frame_path, 'frame_%04d.jpg'))
    ...
    #             # fig_tmp = go.Figure(fig)
    #             # fig_tmp = fig.deepcopy()
    #             # fig_joints.append(fig_tmp)
    #             fig_tmp = fig
    ...
    #             # Using the numpy stack to make the figures aligned horizontal
    #             # fig_joints.append(image)
    #             # new_image = np.hstack(fig_joints)
